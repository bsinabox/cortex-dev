...
export function PipelineBoard({ plans: initialPlans, singles: initialSingles }: PipelineBoardProps) {
  ...
  const { livePlans, liveSingles, fullBatchStats } = useMemo(() => {
    ...
    // Compute full batch stats BEFORE active filter so progress is accurate
    const fullBatchStats = new Map<string, { total: number; done: number }>();
    for (const item of filtered) {
      if (item.batch_id) {
        const stats = fullBatchStats.get(item.batch_id) ?? { total: 0, done: 0 };
        stats.total++;
        if (DONE_STATUSES.includes(item.status)) stats.done++;
        fullBatchStats.set(item.batch_id, stats);
      }
    }
    ...
    // FIX 1: filter inactive items WITHIN each plan, then drop empty plans
    if (activeOnly) {
      lp = lp.map(p => ({ ...p, items: p.items.filter(isActiveItem) }))
             .filter(p => p.items.length > 0);
      sng = sng.filter(isActiveItem);
    }

    lp.sort((a, b) => b.items.length - a.items.length);
    return { livePlans: lp, liveSingles: sng, fullBatchStats };
  ...
  return (
    <div onTouchStart={onTS} onTouchMove={onTM} onTouchEnd={onTE}>
          ...
          {personSingles.length > 0 && (
            <>
              ...
              {Array.from(batchGroups.entries()).map(([batchId, items]) => (
                <BatchGroup key={batchId} batchId={batchId} items={items} person={person} batchStats={fullBatchStats} />
              ))}
      ...
      {/* ─── DRILL VIEW ─── */}
      {drillPlan && (
        <DrillView plan={drillPlan} person={person} onBack={() => setDrillId(null)} onReassign={handleReassign} batchStats={fullBatchStats} />
      )}
    </div>
  );
}
...
/* ─── Batch Group (collapsible, for grid view singles) ─── */

function BatchGroup({ batchId, items, person, batchStats }: {
  batchId: string; items: PipelineItem[]; person: string;
  batchStats: Map<string, { total: number; done: number }>;
}) {
  const [collapsed, setCollapsed] = useState(false);
  const stats = batchStats.get(batchId);
  const done = stats?.done ?? items.filter(i => DONE_STATUSES.includes(i.status)).length;
...
/* ─── Drill Batch Group (collapsible, for drill view sections) ─── */

function DrillBatchGroup({ batchId, items, person, batchStats }: {
  batchId: string; items: PipelineItem[]; person: string;
  batchStats?: Map<string, { total: number; done: number }>;
}) {
  const [collapsed, setCollapsed] = useState(false);
  ...
  return (
    <div className="mb-1">
      <button onClick={() => setCollapsed(!collapsed)}
        className="mb-0.5 flex w-full items-center gap-2 rounded-[6px] bg-[var(--muted)] px-2.5 py-1 text-left">
        <span className="text-[10px] font-bold">{formatBatchId(batchId)}</span>
        <span className="text-[9px] text-[var(--muted-foreground)]">{done}/{total} complete</span>
        <svg className={`ml-auto h-2.5 w-2.5 text-[var(--muted-foreground)] transition-transform ${collapsed ? '' : 'rotate-90'}`}
          fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" d="m8.25 4.5 7.5 7.5-7.5 7.5" />
        </svg>
      </button>
...
function DrillView({ plan, person, onBack, onReassign, batchStats }: {
...
function ItemSection({ label, color, pulse, items, person, subtitle, collapsed: defaultCollapsed, batchStats }: {
  ...
  // Sub-group items by batch_id (FIX 2: component-backed batches get grouping)
  const { batches, unbatched } = useMemo(() => {
    ...
    return { batches: batchMap, unbatched };
  }, [items]);
  ...
  return (
    <div className="mb-2">
      ...
      {!collapsed && (
        <>
          {Array.from(batches.entries()).map(([batchId, bItems]) => (
            <DrillBatchGroup key={batchId} batchId={batchId} items={bItems} person={person} batchStats={batchStats} />
          ))}
          {unbatched.map(item => (
            <DrillItem key={item.id} item={item} person={person} />
          ))}
