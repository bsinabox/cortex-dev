...
export function PipelineBoard({ plans: initialPlans, singles: initialSingles }: PipelineBoardProps) {
  ...
  // Batch grouping for singles — group ALL items first so progress is correct,
  // then filter display by activeOnly
    ...
    <div onTouchStart={onTS} onTouchMove={onTM} onTouchEnd={onTE}>
      ...
      {!drillPlan && (
        <div>
          ...
          {(ungroupedSingles.length > 0 || Array.from(batchGroups.values()).some(items => !activeOnly || items.some(isActiveItem))) && (
            <>
      ...
      {/* ─── DRILL VIEW ─── */}
      {drillPlan && (
        <DrillView plan={drillPlan} person={person} activeOnly={activeOnly} onBack={() => setDrillId(null)} onReassign={handleReassign} />
      )}
    </div>
  );
}
...
function BatchGroup({ batchId, items, person, activeOnly }: { batchId: string; items: PipelineItem[]; person: string; activeOnly: boolean }) {
  const [collapsed, setCollapsed] = useState(false);
  const done = items.filter(i => DONE_STATUSES.includes(i.status)).length;
  const visibleItems = activeOnly ? items.filter(isActiveItem) : items;
...
/* ─── Drill Batch Group (uses DrillItem instead of SingleRow) ─── */

function DrillBatchGroup({ batchId, allItems, visibleItems, person }: { batchId: string; allItems: PipelineItem[]; visibleItems: PipelineItem[]; person: string }) {
  const [collapsed, setCollapsed] = useState(false);
...
function DrillView({ plan, person, activeOnly, onBack, onReassign }: { plan: BuildPlan; person: string; activeOnly: boolean; onBack: () => void; onReassign: (componentId: string, newOwner: string) => void }) {
  const s = calcPlan(plan.items, person);
  ...
  // All non-terminal items (for batch progress and categories)
  const allNonTerminal = plan.items.filter(i => !['cancelled', 'failed'].includes(i.status));
  // Items filtered by activeOnly for display
  const displayedItems = activeOnly ? allNonTerminal.filter(isActiveItem) : allNonTerminal;

  // Batch grouping for component items
  const drillBatches = useMemo(() => {
    const groups = new Map<string, { allItems: PipelineItem[]; visibleItems: PipelineItem[] }>();
    for (const item of allNonTerminal) {
      if (item.batch_id) {
        const entry = groups.get(item.batch_id) ?? { allItems: [], visibleItems: [] };
        entry.allItems.push(item);
        groups.set(item.batch_id, entry);
      }
    }
    for (const item of displayedItems) {
      if (item.batch_id) {
        const entry = groups.get(item.batch_id);
        if (entry) entry.visibleItems.push(item);
      }
    }
    return groups;
  }, [allNonTerminal, displayedItems]);

  // Unbatched items for category sections
  const unbatchedDisplayed = displayedItems.filter(i => !i.batch_id);

  const actionItems = unbatchedDisplayed.filter(i => isAction(i.status, person));
  const autoItems = unbatchedDisplayed.filter(i => isAutonomous(i.status));
  const doneItems = unbatchedDisplayed.filter(i => DONE_STATUSES.includes(i.status));
  const blockedItems = unbatchedDisplayed.filter(i => BLOCKED_STATUSES.includes(i.status));
  const queueItems = unbatchedDisplayed.filter(i => QUEUE_STATUSES.includes(i.status) && !isAction(i.status, person));
  const otherItems = unbatchedDisplayed.filter(i =>
  ...
  return (
    <div>
      ...
      {/* ── BATCH GROUPS ── */}
      {Array.from(drillBatches.entries()).map(([batchId, { allItems, visibleItems }]) => (
        visibleItems.length > 0 && (
          <DrillBatchGroup key={batchId} batchId={batchId} allItems={allItems} visibleItems={visibleItems} person={person} />
        )
      ))}
