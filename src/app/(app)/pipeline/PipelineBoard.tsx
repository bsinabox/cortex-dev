...
export function PipelineBoard({ plans: initialPlans, singles: initialSingles }: PipelineBoardProps) {
  ...
  // Batch totals from ALL items (unfiltered) for accurate progress (QA fix #3)
  ...
  const { livePlans, liveSingles } = useMemo(() => {
    ...
    // QA fix #1: filter inactive items WITHIN each plan, not just whole plans
    if (activeOnly) {
      lp = lp
  ...
  // Batch grouping for singles (QA fix #2: also handle component batches in DrillView)
          ...
          {personSingles.length > 0 && (
            <>
              ...
              {Array.from(batchGroups.entries()).map(([batchId, items]) => {
                const bt = batchTotals.get(batchId);
                return (
                  <BatchGroup key={batchId} batchId={batchId} items={items} person={person}
                    totalCount={bt?.total ?? items.length} doneCount={bt?.done ?? 0} />
                );
              })}
...
function DrillBatchGroup({ batchId, items, person, totalCount, doneCount }: {
  batchId: string; items: PipelineItem[]; person: string;
  totalCount: number; doneCount: number;
}) {
  const [collapsed, setCollapsed] = useState(false);
...
function DrillView({ plan, person, onBack, onReassign, batchTotals }: {
  plan: BuildPlan; person: string; onBack: () => void;
  onReassign: (componentId: string, newOwner: string) => void;
  batchTotals: Map<string, { total: number; done: number }>;
}) {
  const s = calcPlan(plan.items, person);
  ...
  // QA fix #2: separate batched items from non-batched for batch grouping
  const { batchedGroups, unbatchedItems } = useMemo(() => {
    const groups = new Map<string, PipelineItem[]>();
    ...
    return { batchedGroups: groups, unbatchedItems: unbatched };
  }, [activeItems]);

  // Categorize only non-batched items
  const actionItems = unbatchedItems.filter(i => isAction(i.status, person));
  const autoItems = unbatchedItems.filter(i => isAutonomous(i.status));
  const doneItems = unbatchedItems.filter(i => DONE_STATUSES.includes(i.status));
  const blockedItems = unbatchedItems.filter(i => BLOCKED_STATUSES.includes(i.status));
  const queueItems = unbatchedItems.filter(i => QUEUE_STATUSES.includes(i.status) && !isAction(i.status, person));
  const otherItems = unbatchedItems.filter(i =>
  ...
  return (
    <div>
      ...
      {/* ── BATCH GROUPS (QA fix #2: component-backed batches get headers) ── */}
      {batchedGroups.size > 0 && (
        <>
          {Array.from(batchedGroups.entries()).map(([batchId, items]) => {
            const bt = batchTotals.get(batchId);
            return (
              <DrillBatchGroup key={batchId} batchId={batchId} items={items} person={person}
                totalCount={bt?.total ?? items.length} doneCount={bt?.done ?? 0} />
            );
          })}
        </>
